...
![600](https://pic-1315225359.cos.ap-shanghai.myqcloud.com/20260224163024.png)
...
可以将**数据表**和**行名称**添加到 CommonButtonBase 等控件中的 触发输入动作（Triggering Input Action）。之后，按下该动作所关联的按钮会触发Common UI按钮；
...
控制器数据的参数：
	...
	输入刷按键组`Input Brush Key Sets`：用于多个按键映射到单个UI元素，比如十字键或者其他可能映射到多个轴的输入；

...
5. **优先级**
	**转换规则的优先级（Priority Order）** 主要用来解决一个核心矛盾：**当一个状态（State）同时有多个导线（Transition）指向不同的目标时，引擎应该先听谁的？**

	假设你的角色处于 `Idle`（待机）状态，这时候有三条转换导线连出去：
	1.  **Idle -> Walk**（条件：速度 > 0）
	2.  **Idle -> Jump**（条件：按下了跳跃键）
	3.  **Idle -> Death**（条件：血量 <= 0）
	
	**冲突发生了：**
	如果在一帧内，角色**正好在起跑**（速度 > 0），同时**又被敌人一枪打死**（血量 <= 0）。
	*   如果没有优先级，引擎可能会产生随机行为，或者执行它在内部列表中最先找到的那条规则（通常是先连的那条线）。
	*   你肯定希望“死亡”动作拥有最高话语权。这时，你就需要把 `Idle -> Death` 的 **Priority Order** 设为 `0`（最高）。
